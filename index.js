...
export function loginGoogle(errorGooFbkModal) {
  ...
  btngoGoogle.addEventListener("click", (e) => {
    e.preventDefault();
    firebase
      .auth() //Se llama a la func autenticar
      .signInWithPopup(provider) //Se va a usar una popUp y se va a loguear con la var provider que es google
      .then(function (result) {
        //lo que se hace cuando el usuario ya inicio sension y ya dio permisos, nos dio su info
...
export function createUser(
  ...
  if (
    expRegPass.test(newPassword) === true &&
    expRegEmail.test(newEmail) === true
  ) {
    firebase
      .auth()
      ...
      .catch(function (error) {
        //Si la cuenta se ha creado se muestra el error
...
function saveEmailBD(newName, newEmail, newPassword, user) {
  ...
  docRef
    .set({
      ...
      interests: "Interés",
      photo:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcS_nu03Fphc39lFDYvqsGjluX8keorjzG79akJclhK2fS2SwLKo&usqp=CAU",
      uid: user.user.uid,
    })
    .then(function () {
      console.log("Los datos se guardaron");
