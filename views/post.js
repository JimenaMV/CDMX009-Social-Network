import { root, navBar } from "../main.js";
...
export function renderPostView() {
  const posts = `<section id="container">
...
function showPosts(sectionPosts) {
  firebase
    ...
    .onSnapshot((snap) => {
      ...
      <a href="#" dataid="${
        doc.id
      }" class="likes btnLike"><i class="fas fa-heart"></i></a>
      <div class="resultCounter counter">${doc.data().likes}</div>
      <a href="#" dataid="${
        doc.id
      }" class="comentPost counter"><i class="far fa-comment-alt"></i></a>
      </div>
    </div>`;
        ...
        if (doc.data().img != undefined && doc.data().photo != undefined) {
          let renderPosts = `<div class="postContent" dataid="${doc.id}">
               <img max- width="70" class="imgUserPost" src="${
                 doc.data().photo
               }"/>
      <p class="subtitle is-6 des"> ${doc.data().user}</p>
      <p class="subtitle is-3 des">${doc.data().title}</p>
